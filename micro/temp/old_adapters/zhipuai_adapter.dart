...
class ZhipuAIAdapter implements ProviderAdapter {
  ...
  Future<micro.ChatMessage> sendMessage({
    ...
    try {
      ...
      // Defensive null checking for response and output
      if (response == null) {
        throw Exception('Null response from ZhipuAI model');
      }
      
      final output = response.output;
      if (output == null) {
        throw Exception('Null output from ZhipuAI response');
      }
      
      // Extract content as String - LangChain's AIChatMessage.content is already String
      String contentText;
      if (output is AIChatMessage) {
        contentText = output.content;
      } else if (output is SystemChatMessage) {
        contentText = output.content;
      } else if (output is HumanChatMessage) {
        // HumanChatMessage.content is ChatMessageContent, convert to string
        contentText = output.content.toString();
      } else {
        // Fallback: convert to string
        contentText = output.toString();
      }
      
      if (contentText.isEmpty) {
        throw Exception('Empty content in ZhipuAI response');
      }
      
      return _convertResponseToMicro(contentText);
    } catch (e) {
